...
Model modelosCasillasD[40];
Model modelosCasillasN[40];
...
// Declaración de la cámara actual y bandera para cambio de listener
TipoCamara camaraActual = Isometrica;
bool actualizarListener = true;  // Nueva bandera

// Función para cambiar la vista y la posición del Listener
void cambiarVistaCamara(Camera* camera) {
	if (camaraActual == TerceraPersona) {
		camaraActual = Aerea;
		camera->setVistaAerea(275.0f);
...
// Función para actualizar la posición del Listener dinámicamente
void actualizarPosicionListener(Camera* camera) {
	if (camaraActual == TerceraPersona) {
		// Seguir la posición del modelo en tercera persona
		sf::Listener::setPosition(bobPosition.x, bobPosition.y, bobPosition.z);
	}
	else if (camaraActual == Aerea) {
		// Seguir la posición actual de la cámara en vista aérea
		glm::vec3 posCam = camera->getPosition();
		sf::Listener::setPosition(posCam.x, posCam.y, posCam.z);
	}
	else if (camaraActual == Isometrica) {
		// Posición fija para vista isométrica
		sf::Listener::setPosition(-150.0f, 120.0f, 150.0f);
	}
}
...
int obtenercasilla(float movZ, float movX) {
	...
	if (intMovX == 0 && intMovZ <= 170) {
		casilla = (movZ * 10) / 180;
	}
	else if (intMovZ > 160 && intMovX >= -180) {
		casilla = 10 + ((-movX * 10) / 180) -0.5;
	}
	else if (intMovX < -180 && intMovZ > 0) {
		casilla = 30 + (-movZ * 10) / 180;
	}
	else if (intMovZ < 20 && intMovX <= 0) {
...
sf::Sound esponjaPasos;
sf::SoundBuffer esponjaPasos_buffer;
void inicializarSonido(){
	esponjaPasos_buffer.loadFromFile("Audio/Esponja.wav");
	esponjaPasos.setPitch(1.0f);
	esponjaPasos.setVolume(5.0f);
	esponjaPasos.setBuffer(esponjaPasos_buffer);
	esponjaPasos.setMinDistance(2.5f);
	esponjaPasos.setAttenuation(2.0f);
	esponjaPasos.setLoop(true);
}
...
void moverJugador(float deltaTime) {
	...
	if (casillaActual < TOTAL_CASILLAS) {
		...
		if (casillaActual == casillaObjetivo) {
			...
			cicloCompleto = false;
			if (esponjaPasos.getStatus() == sf::Sound::Playing) {
				esponjaPasos.stop();
			}
	...
	if (casillaActual >= TOTAL_CASILLAS) {
		...
		movZ = 0;
		movX = 0;
		// Detener el sonido si se regresa a la casilla de inicio
		if (esponjaPasos.getStatus() == sf::Sound::Playing) {
			esponjaPasos.stop();
		}
	}
...
int main()
{
	...
	Gengar_M = Model();
	Gengar_M.LoadModel("Models/gengar.obj");
	FemTrainer_M = Model();
	FemTrainer_M.LoadModel("Models/FemTrainer.obj");
	Sonia_M = Model();
	Sonia_M.LoadModel("Models/Sonia.obj");
	Snorlax_M = Model();
	Snorlax_M.LoadModel("Models/Snorlax.obj");
	PatricioVend_M = Model();
	PatricioVend_M.LoadModel("Models/PatricioVend.obj");
	Flareon_M = Model();
	Flareon_M.LoadModel("Models/Flareon.obj");
	Blastoise_M = Model();
	Blastoise_M.LoadModel("Models/Blastoise.obj");
	PikachuAlt_M = Model();
	PikachuAlt_M.LoadModel("Models/Pikachu.obj");
	Casas = Model();
	Casas.LoadModel("Models/Casas.obj");
	Casas2 = Model();
	Casas2.LoadModel("Models/Casas2.obj");
	Cartel2 = Model();
	Cartel2.LoadModel("Models/Cartel2.obj");
	Basura = Model();
	...
	Esponjatron_M = Model();
	Esponjatron_M.LoadModel("Models/Esponjatron.obj");
	//Modelos Pokemon
	RedCabeza_M = Model();
	...
	////////////////////////////////////////////////////////////////////////////////////////
	modelosCasillasN[0] = Rhydon_MNoche;
	modelosCasillasN[1] = BobNoche;
	modelosCasillasN[2] = ArbolNoche;
	modelosCasillasN[3] = GaryNoche;
	modelosCasillasN[4] = CasaBobNoche;
	modelosCasillasN[5] = PatricioNoche;
	modelosCasillasN[6] = KelpNoche;
	modelosCasillasN[7] = MedusaNoche;
	modelosCasillasN[8] = CasaCalamardoNoche;
	modelosCasillasN[9] = CalamardoNoche;
	modelosCasillasN[10] = FlorNoche;
	modelosCasillasN[11] = PezNoche;
	modelosCasillasN[12] = CasaPatricioNoche;
	modelosCasillasN[13] = PlanktonNoche;
	modelosCasillasN[14] = BurbujaNoche;
	modelosCasillasN[15] = CrustaceoNoche;
	modelosCasillasN[16] = Pikachu_MNoche;
	modelosCasillasN[17] = CoralNoche;
	modelosCasillasN[18] = Pez2Noche;
	modelosCasillasN[19] = EdificioPoke_MNoche;
	modelosCasillasN[20] = Lapras_MNoche;
	modelosCasillasN[21] = OstraNoche;
	modelosCasillasN[22] = Jessie_MNoche;
	modelosCasillasN[23] = GymPalace_MNoche;
	modelosCasillasN[24] = Charmander_MNoche;
	modelosCasillasN[25] = TreeOfLife_MNoche;
	modelosCasillasN[26] = James_MNoche;
	modelosCasillasN[27] = Electrode_MNoche;
	modelosCasillasN[28] = PineTree_MNoche;
	modelosCasillasN[29] = Giovanni_MNoche;
	modelosCasillasN[30] = CasaPlayer_MNoche;
	modelosCasillasN[31] = Weezing_MNoche;
	modelosCasillasN[32] = ColorfulPlant_MNoche;
	modelosCasillasN[33] = Jenny_MNoche;
	modelosCasillasN[34] = PokeMart_MNoche;
	modelosCasillasN[35] = Lechonk_MNoche;
	modelosCasillasN[36] = BerryTree_MNoche;
	modelosCasillasN[37] = Joy_MNoche;
	modelosCasillasN[38] = CentroPoke_MNoche;
	modelosCasillasN[39] = Azumarill_MNoche;
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	////////////////////////////////////////////////////////////////////////////////////////
	modelosCasillasD[0] = Rhydon_MBase;
	modelosCasillasD[1] = BobBase;
	modelosCasillasD[2] = ArbolBase;
	modelosCasillasD[3] = GaryBase;
	modelosCasillasD[4] = CasaBobBase;
	modelosCasillasD[5] = PatricioBase;
	modelosCasillasD[6] = KelpBase;
	modelosCasillasD[7] = MedusaBase;
	modelosCasillasD[8] = CasaCalamardoBase;
	modelosCasillasD[9] = CalamardoBase;
	modelosCasillasD[10] = FlorBase;
	modelosCasillasD[11] = PezBase;
	modelosCasillasD[12] = CasaPatricioBase;
	modelosCasillasD[13] = PlanktonBase;
	modelosCasillasD[14] = BurbujaBase;
	modelosCasillasD[15] = CrustaceoBase;
	modelosCasillasD[16] = Pikachu_MBase;
	modelosCasillasD[17] = CoralBase;
	modelosCasillasD[18] = Pez2Base;
	modelosCasillasD[19] = EdificioPoke_MBase;
	modelosCasillasD[20] = Lapras_MBase;
	modelosCasillasD[21] = OstraBase;
	modelosCasillasD[22] = Jessie_MBase;
	modelosCasillasD[23] = GymPalace_MBase;
	modelosCasillasD[24] = Charmander_MBase;
	modelosCasillasD[25] = TreeOfLife_MBase;
	modelosCasillasD[26] = James_MBase;
	modelosCasillasD[27] = Electrode_MBase;
	modelosCasillasD[28] = PineTree_MBase;
	modelosCasillasD[29] = Giovanni_MBase;
	modelosCasillasD[30] = CasaPlayer_MBase;
	modelosCasillasD[31] = Weezing_MBase;
	modelosCasillasD[32] = ColorfulPlant_MBase;
	modelosCasillasD[33] = Jenny_MBase;
	modelosCasillasD[34] = PokeMart_MBase;
	modelosCasillasD[35] = Lechonk_MBase;
	modelosCasillasD[36] = BerryTree_MBase;
	modelosCasillasD[37] = Joy_MBase;
	modelosCasillasD[38] = CentroPoke_MBase;
	modelosCasillasD[39] = Azumarill_MBase;
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//Modelos Cocinados
	BobCocinado = Model();
	...
	velocidadRotacion = 135.0f;  // Grados por segundo
	velocidadMovimiento = 0.75f;  // Velocidad de desplazamiento vertical
	...
	sound_buffer.loadFromFile("Audio/SoundtrackCGIHC.wav");
	...
	sf::Sound bob;
	sf::SoundBuffer bob_buffer;
	bob_buffer.loadFromFile("Audio/Esponjatron.wav");
	bob.setPosition(120.0f, 2.0f, 120.0f);
	bob.setVolume(250.0f);
	bob.setBuffer(bob_buffer);
	bob.setMinDistance(2.0f);
	bob.setAttenuation(22.0f);
	bob.setLoop(true);
	bob.play();


	sf::Sound bmw;
	sf::SoundBuffer bmw_buffer;
	bmw_buffer.loadFromFile("Audio/NFSMW.wav");
	bmw.setVolume(250.0f);
	bmw.setBuffer(bmw_buffer);
	bmw.setMinDistance(3.5f);
	bmw.setAttenuation(7.0f);
	bmw.setLoop(true);
	bmw.play();
	...
	{
		//sf::Listener::setPosition(-90.0f + movZ, 1.0f, 95.0f + movX);
		UpdateSunDirection();
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//SkyBox
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		// 
	 // Actualizar la posición de "bob" (modelo) o cámara en cada iteración
		bobPosition = glm::vec3(-90.0f + movZ, 1.0f, 95.0f + movX);

		// Detectar cambio de cámara con tecla C
		if (mainWindow.getsKeys()[GLFW_KEY_C]) {
			cambiarVistaCamara(&camera);
			mainWindow.getsKeys()[GLFW_KEY_C] = false;
		}

		// Actualizar la posición del listener en cada cuadro
		actualizarPosicionListener(&camera);

		// Configurar vistas de cámara y listener según la cámara activa
		...
		//Movimiento de Charizard alrededeor del tablero
			...
			bmw.setPosition(150.0f + movObjX_bote, 3.0f, 160.0f + movObjZ_bote);
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//Bob
		 if (camaraActual == TerceraPersona) {    
        camera.setVistaTerceraPersona(bobPosition, distanceBehind, height, yawBob);
		...
		if (isNight == true) {//Es de noche, entonces mandamos las luces al shader
			...
			else if (nuevaCasilla >= 1 && nuevaCasilla <= 9) {
			...
			else if (nuevaCasilla >= 11 && nuevaCasilla <= 19) {
			...
			else if (nuevaCasilla >= 21 && nuevaCasilla <= 29) {
			...
			else if (nuevaCasilla >= 31 && nuevaCasilla <= 39) {
		...
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Esponjatron_M.RenderModel();
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(115.0f, 0.25f, 125.0f));
		...
		if (isNight == false) {
			...
			for (int i = 0; i < TOTAL_CASILLAS; i++) {
				...
				Material_brillante.UseMaterial(uniformSpecularIntensity, uniformShininess);
				glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
				modelosCasillasD[i].RenderModel();
			}
		}
			...
			for (int i = 0; i < TOTAL_CASILLAS; i++) {
				...
				Material_brillante.UseMaterial(uniformSpecularIntensity, uniformShininess);
				glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
				modelosCasillasN[i].RenderModel();
			}
		...
		esponjaPasos.setPosition(-90.0f + movZ, 1.0f, 95.0f + movX);
...
//////////////////////////MODELOS TABLERO/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	////////////////////////MODELOS ALREDEDOR DEL TABLERO///////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		//Casa1
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(190, 1.5f, 45.0));
		model = glm::rotate(model, -90 * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
		Material_brillante.UseMaterial(uniformSpecularIntensity, uniformShininess);
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		//Casa3 
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(-210, 1.5f, -50.0));
		model = glm::rotate(model, -90 * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
		Material_brillante.UseMaterial(uniformSpecularIntensity, uniformShininess);
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		//Gengar
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(-60.0f, 1.5f, -185.0f));
		//model = glm::scale(model, glm::vec3(2.5f, 2.5f, 2.5f));
		model = glm::rotate(model, -90 * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Gengar_M.RenderModel();
		
		//Sonia
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(110.0f, 1.5f, -185.0f));
		//model = glm::scale(model, glm::vec3(2.5f, 2.5f, 2.5f));
		//model = glm::rotate(model, -90 * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Sonia_M.RenderModel();
		...
		//Snorlax
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(-150.0f, 1.5f, -185.0f));
		model = glm::scale(model, glm::vec3(2.5f, 2.5f, 2.5f));
		model = glm::rotate(model, -90 * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Snorlax_M.RenderModel();

		//Patricio Vendedor
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(190, 1.5f, 60.0));
		model = glm::rotate(model, 180 * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
		Material_brillante.UseMaterial(uniformSpecularIntensity, uniformShininess);
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		PatricioVend_M.RenderModel();


		//Blastoise
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(190.0, 1.5f, -45.0));
		model = glm::scale(model, glm::vec3(1.5f, 1.5f, 1.5f));
		Material_brillante.UseMaterial(uniformSpecularIntensity, uniformShininess);
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Blastoise_M.RenderModel();
		
		//Flareon
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(190.0, 1.5f, -30.0));
		model = glm::rotate(model, 180 * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
		Material_brillante.UseMaterial(uniformSpecularIntensity, uniformShininess);
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Flareon_M.RenderModel();

		//Pikachu
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(105.0f, 1.5f, -185.0f));
		//model = glm::scale(model, glm::vec3(2.5f, 2.5f, 2.5f));
		//model = glm::rotate(model, -90 * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		PikachuAlt_M.RenderModel();


		//Entrenadora 
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(-70.0f, 1.5f, -185.0f));
		//model = glm::scale(model, glm::vec3(2.5f, 2.5f, 2.5f));
		model = glm::rotate(model, 90 * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		FemTrainer_M.RenderModel();
		...
		//Casas 
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(300, 1.5f, 220.0));
		...
		//Casas Calamardo
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(-210, 1.5f, 80.0));
		model = glm::scale(model, glm::vec3(2.0f, 2.0f, 2.0f));
		model = glm::rotate(model, 90 * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Casas2.RenderModel();
		...
		//Basura
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(-200, 1.5f, -100.0));
